...
function config(){
  ...
  const r={
    ...
    effects_profile: has('audio_profile')?val('effects_profile'):'',
    volume_gain_db: has('audio_profile')?(Number(val('volume_gain_db','0'))||0):0,
  };
  return r;
}
...
function applySample(sample,button,quiet=false){
  ...
  if($('effects_profile')) $('effects_profile').value=c.effects_profile||'';
  if($('volume_gain_db')) $('volume_gain_db').value=c.volume_gain_db!=null?String(c.volume_gain_db):'0';
